...
const std::string EE_FORCE_SENSOR_FORCE_KEY = "sai2::optoforceSensor2::6Dsensor::force";
...
void Run6DSensorExample(OptoDAQ & p_optoDAQ)
{
    ...
    Eigen::VectorXd force_offset = Eigen::VectorXd::Zero(6);

    OptoPackage6D optoPackage;
	int iReadSize = ReadPackage6D(p_optoDAQ, optoPackage);	
	processRaw6DSensorData(optoPackage, force_offset);
...
//	do {
	while(true)
	{
		...
		force_raw = force_raw - force_offset;
		...
		// std::cout << "force filtered: " << force_filtered.transpose() << std::endl;
		// std::cout << "force raw: " << force_raw.transpose() << std::endl;
...
int main()
{
	...
	int iPortIndex = 1;  // The index of the port which will be opened
